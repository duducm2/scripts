...
- **Keywords (`keywords`):** Association pairs from concept pipe-chunks (thought groups). Each pair is `Keyword | RecognizableWord` joined across pairs with ` || ` (max **10**). **Keyword** = tangible memory anchor (physical object, action, actor, cartoon character). **RecognizableWord** = prominent word taken from that thought group (prefer short, everyday forms). Displayed under Concept in practice MD and the web SPA. Never invent pairs for Note text.
...
3. **Keywords:** After Context, list association pairs (one per line as `Keyword | RecognizableWord`, or CSV `keywords` as `a | b || c | d`). Supports dual-access cues under Concept. Cap 10 pairs.
