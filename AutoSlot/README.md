...
Place: empty monitor → maximize; else free half → 50/50; else maximize in place. Close/minimize of a pair **heals** (maximizes) the leftover companion. **Y**: lone half + free slot → maximize that window; lone max → 50/50 with background. Details and busy overlays: canon doc above.
