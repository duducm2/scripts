        ...
        <div class="panel chart-cell budget-panel">
          ...
          <div class="funds-compare" id="fundsCompare" aria-live="polite">
            <div class="funds-compare-title-row">
              <div class="funds-compare-title">Funds</div>
            ...
            <div class="funds-block" id="fundsAvailableBlock">
              <div class="funds-block-head">
                <span class="funds-legend-item">
                  <span class="funds-legend-swatch available"></span>
                  Available cash <span class="funds-compare-amounts" id="fundsAvailableVal">—</span>
                </span>
              </div>
              <div class="funds-compare-track">
                <div class="funds-bar funds-bar-available" id="fundsBarAvailable" style="width:0%"></div>
              </div>
              <div class="funds-compare-meta" id="fundsAvailableMeta"></div>
            </div>
            <div class="funds-block" id="fundsBudgetBlock">
              <div class="funds-block-head">
                <span class="funds-legend-item">
                ...
                <span class="funds-legend-item" id="fundsProjectedSaveItem">
                  ...
                  <span class="funds-compare-amounts" id="fundsProjectedSaveVal">—</span>
                </span>
              </div>
              <div class="funds-compare-track">
                <div class="funds-bar funds-bar-planned" id="fundsBarPlanned" style="width:100%"></div>
                <div class="funds-bar funds-bar-spent" id="fundsBarSpent" style="width:0%"></div>
              </div>
              <div class="funds-compare-meta" id="fundsBudgetMeta"></div>
            </div>
            {card_avail_block}
          </div>
...
<head>
  ...
  <style>
    ...
    .funds-block {{ margin:0 0 10px; }}
    .funds-block-head {{
      display:flex; align-items:center; justify-content:space-between;
      gap:10px; flex-wrap:wrap; font-size:12px; color:var(--muted); margin-bottom:4px;
    }}
    .funds-legend-item {{
    ...
    .funds-bar-available {{
      background:#3498db; opacity:.9; z-index:1;
    }}
    .funds-bar-planned {{
      background:#2ecc71; opacity:.28; z-index:1;
    }}
    .funds-bar-spent {{
      background:#f1c40f; opacity:.95; z-index:2;
    }}
    .funds-compare.overspent .funds-bar-spent {{ background:#e74c3c; opacity:.9; }}
    .funds-compare-meta {{
      color:var(--muted2); font-size:11px; margin-top:4px;
    }}
    .funds-compare.overspent #fundsBudgetMeta {{ color:#e74c3c; font-weight:600; }}
    .budget-calc-wrap {{
...
/** Unspent plan (planned − spent). Positive = projected save; negative = shortfall. */
function projectedSaveHeadroom() {{
  return plannedTotalForCurrentMonth() - spentTotalForCurrentMonth();
}}
function refreshFundsCompare() {{
  ...
  const checkingRaw = Number(RAW.liquidAccountBal);
  const checking = Number.isFinite(checkingRaw) ? checkingRaw : 0;
  const cardSpentRaw = Number(RAW.liquidCardSpent);
  const cardSpend = Number.isFinite(cardSpentRaw) ? cardSpentRaw : 0;
  const planned = plannedTotalForCurrentMonth();
  const spent = spentTotalForCurrentMonth();
  const availScale = Math.max(checking, Math.abs(avail), 0.01);
  const availPct = Math.max(0, Math.min(100, (Math.max(0, avail) / availScale) * 100));
  const spentPct = planned > 0
    ? Math.min(100, (spent / planned) * 100)
    : (spent > 0 ? 100 : 0);
  const headroom = projectedSaveHeadroom();
  const overspent = headroom < -0.001 && planned > 0;
  const onTrack = !overspent;
  root.classList.toggle('deficit', overspent);
  root.classList.toggle('ok', onTrack && planned > 0);
  ...
  const availMeta = document.getElementById('fundsAvailableMeta');
  const budgetMeta = document.getElementById('fundsBudgetMeta');
  ...
  if (saveEl) {{
    saveEl.textContent = overspent
      ? ('−' + formatBrl(-headroom))
      : formatBrl(Math.max(0, headroom));
  }}
  if (saveLbl) saveLbl.textContent = overspent ? 'Shortfall' : 'Projected save';
  if (barAvail) barAvail.style.width = availPct.toFixed(1) + '%';
  if (barPlan) barPlan.style.width = planned > 0 || spent > 0 ? '100%' : '0%';
  if (barSpent) barSpent.style.width = spentPct.toFixed(1) + '%';
  if (availMeta) {{
    availMeta.textContent = 'Checking ' + formatBrl(checking)
      + ' − card spent ' + formatBrl(cardSpend);
  }}
  if (budgetMeta) {{
    if (planned <= 0 && spent <= 0) {{
      budgetMeta.textContent = 'No planned budget this month';
    }} else if (overspent) {{
      budgetMeta.textContent = 'Exceeded planned by ' + formatBrl(-headroom)
        + ' · ' + (planned > 0 ? (spent / planned * 100).toFixed(0) + '% of planned' : 'no plan');
    }} else {{
      budgetMeta.textContent = 'Unspent plan ' + formatBrl(headroom)
  ...
  if (tip) {{
    ...
    const tipCardSpend = Number.isFinite(cardSpent) ? cardSpent : 0;
    ...
    if (cardRows.length) {{
      ...
      parts.push('<div class="calc-line">Cards · spent ' + formatBrl(tipCardSpend)
        + ' · limit ' + formatBrl(limit) + '</div>');
    }}
    parts.push('<div class="calc-line calc-total">Available = checking '
      + formatBrl(bal) + ' − card spent ' + formatBrl(tipCardSpend)
      + ' = ' + formatBrl(avail) + '</div>');
