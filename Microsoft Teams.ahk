...
+o:: {
    ...
    if !hwnd {
        ShowCenteredOverlay(WinGetID("A"), "⚠ NO TEAMS CHAT WINDOW", 2000, BANNER_ACCENT_ERROR)
        return
    }
    try {
        ...
        if !quickViews {
            ShowCenteredOverlay(WinGetID("A"), "❌ QUICK VIEWS NOT FOUND", 2000, BANNER_ACCENT_ERROR)
            return
        }
        quickViews.SelectionItemPattern.Select()
    } catch as e {
        ShowCenteredOverlay(WinGetID("A"), "❌ QUICK VIEWS: " e.Message, 2000, BANNER_ACCENT_ERROR)
    }
}
