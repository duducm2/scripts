...
; From repo root, start daemon first: python infra\python\applauncher_daemon.py
; Run this script (from aux\ or repo root) to measure roundtrip latency and sequence integrity.
...
loop 20 {
    ...
    if (resp.Has("ok") && resp["ok"])
        continue
    MsgBox("Harness: Ping failed or daemon not running. From repo root: python infra\python\applauncher_daemon.py")
    ExitApp 1
}
