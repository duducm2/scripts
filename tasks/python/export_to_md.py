"""Export tasks CSV data to notes Markdown (work.md / punctual.md / habits.md).

Human-readable mirror for the notes repo. Not required to round-trip through migrate_from_md.
"""
...
def write_area(
    ...
    if filt == "habits":
        # Group habitual tasks under recurrence ## headings when possible
...
def main() -> int:
    ...
    ap.add_argument("--habits", required=True, type=Path)
    args = ap.parse_args()
    ...
    print(
        f"Exported: work={args.work} punctual={args.punctual} habits={args.habits} "
