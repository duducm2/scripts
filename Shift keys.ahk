...
#!+w::
{
    ...
    if WinExist("Gmail ahk_exe chrome.exe") {
        ...
        Run "chrome.exe --new-window https://mail.google.com"
...
{
    SetTitleMatchMode 2
    if WinExist("Wikip ahk_exe chrome.exe") {
        WinActivate
        CenterMouse()
    } else {
        Run "chrome.exe --new-window https://www.wikipedia.org/"
        WinWaitActive("Wikip ahk_exe chrome.exe")
        CenterMouse()
    }
}
