...
PowerPoint_GetOneDriveMounts() {
    ...
    ; PowerShell fallback when Loop Reg yields nothing.
...
PowerPoint_CloudUrlToLocalFolder(cloudUrl) {
    ...
    url := PowerPoint_NormalizeFolder(cloudUrl)
...
PowerPoint_ResolveExportFolder(pres) {
    ...
    for cu in cloudUrls {
        ...
        if (syncDir != "" && DirExist(syncDir)) {
            syncFolderOnly := syncDir
            return { folder: syncDir, syncFolder: syncDir, cloudFallback: false }
        }
    }

    return { folder: A_Desktop, syncFolder: syncFolderOnly, cloudFallback: true }
}
...
PowerPoint_SaveAsPdf() {
    ...
    if (moveTarget = "" || moveTarget = A_Desktop)
        moveTarget := ""
