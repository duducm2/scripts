...
- `vendor/UIA-v2/` — UI Automation v2 library (`UIA.ahk`, `UIA_Browse sr.ahk`); see [vendor/UIA-v2/README.md](vendor/UIA-v2/README.md)
...
- **Global AI provider:** `ResolveGlobalAICompanion()` in `lib/CopilotWeb.ahk` — Gemini on personal; at work prefers **Gemini Enterprise** when that window is open (or as the default open target), otherwise M365 Copilot web. Full matrix and Shift-key peers: [docs/global-ai-companion-routing.md](docs/global-ai-companion-routing.md). Optional `COPILOT_WEB_*` URL/title overrides in env.ahk if your tenant differs. Do **not** define `UseCopilotWebForGlobalAI`, `GetGlobalAIProviderLabel`, or `ResolveGlobalAICompanion` in env.ahk (they live in `lib/CopilotWeb.ahk`; duplicates break Act at startup).
...
5. Run `Act.ahk`.
6. Smoke test AI hotkeys (`#!+p` copy; read aloud via D2C **R** / IPC): personal should use Gemini; work should use M365 Copilot web. Smoke test `#!+o` (Utils): cut newest Desktop item and restore previous window.
