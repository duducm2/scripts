...
Before pasting from Utility Shortcuts → Prompts, a 3s banner appears **immediately** after you pick a prompt (before context attach): **[Y]** include human reminders (below `---`), **[Esc]** paste stripped (default), or **[S]** paste stripped and send Enter. **[S]** waits for context uploads to finish and the Send button to enable (per companion: Gemini, Enterprise, or Copilot) before submitting. Attach/paste run after your choice. Shift-keys composer strip still uses `ReplaceComposerWithStrippedReminders`.
