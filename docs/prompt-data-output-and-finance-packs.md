...
**Push (all domains):** Utility Shortcuts top-level `[G]` / category **Push** is the only commit+push entrypoint ([`utility_git_push.ahk`](../Utils/utility_git_push.ahk)). It runs in the background, exports **personal** tasks from `tasks/data/` CSV to `main/punctual.md` in the notes repo via [`export_to_md.py`](../tasks/python/export_to_md.py) (GitHub mirror; CSV remains source of truth), syncs Memory Palace practice+plans MD when `mnemonics/data` is dirty, then commits and pushes **scripts** and **notes**. Finance/Palace/Tasks in-app push keys were removed.
...
**Palace atoms CSV columns** (`ATOMS_HEADERS` / `atoms.csv`): `id`, `beast_id`, `kind`, `zone`, `zone_label`, `concept`, `keywords`, `quote`, `story`, `sensory`, `ipa`, `sort_order`. New or rewritten `concept` values start with a dedicated name group and then bracketed definition groups (`[Name] [chunk] [chunk]`, **3–6** groups); optional ` — Note:` stays outside the brackets. `keywords` stores **exactly one** association pair per bracket group and **3–6 ordered pairs total** matching the group count as `Keyword | RecognizableWord` joined with `||`. Pairs follow left-to-right group order; repeated name/definition terms remain separate. They are shown under Concept in the SPA and practice Markdown as `[word] → [keyword]`. Legacy atom rows remain readable without migration.
