...
**Palace atoms CSV columns** (`ATOMS_HEADERS` / `atoms.csv`): `id`, `beast_id`, `kind`, `zone`, `zone_label`, `concept`, `keywords`, `quote`, `story`, `sensory`, `ipa`, `sort_order`. New or rewritten `concept` values start with a dedicated name group and then bracketed definition groups (`[Name] [chunk] [chunk]`, at most six); optional ` — Note:` stays outside the brackets. `keywords` stores **1–2** association pairs per bracket group and **3–6 ordered pairs total** as `Keyword | RecognizableWord` joined with ` || `. Every group is covered; pairs follow left-to-right group and word order; repeated name/definition terms remain separate. They are shown under Concept in the SPA and practice Markdown as `[word] → [keyword]`. Legacy atom rows remain readable without migration.
