...
| Repeated enumeration | `Ctrl+Alt+Win+6` collects fill candidates **once** per pass (`AutoSlot_CollectFillCandidates` → `g_AutoSlotYBgRows`: hidden first, then visible unslotted); picks consume from that list; one QC backfill for remaining free capacity (empty / half / lone-max) |
