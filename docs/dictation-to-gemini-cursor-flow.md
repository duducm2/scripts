...
**Multi-monitor UI:** While **Send dictation?**, **Response ready**, or the **visible-window paste picker** (**W**) is open, banners and the picker stay on the monitor that contains the **current foreground window**. If you switch focus to a window on another screen, the visible prompt moves to that monitor so it remains in view (implemented via `trackActiveMonitor` on standard bars in `Utils.ahk` / `Gemini.ahk`).
...
flowchart TB
    ...
    BannerCopy --> UserCopy{"P / Y / F / R / W / O / N / Esc / timeout"}
    UserCopy --> |"N_Esc_timeout"| StopCopy["Flow ends no copy"]
    UserCopy --> |"P"| DoCopyOnly["DoCopyCore copy-only"]
...
| 2    | **Response ready — what next?** (5s) | Same strip as `#\!+P` plus **P** Copy-only: **P** Copy, **Y** Desktop, **F** Favorite, **R** Read (omit Enterprise), **W** Paste window, **O** Clip Angel, **N** / **Esc** / timeout = No. Each destination copies the reply first (`DoCopyCore` with `#\!+P`-style quality gates), then the destination action.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | N / Esc / timeout end with no copy. Destination keys copy then act.                                                                                                                                                                                                                                              |
...
| `#!+L`                  | Paste OS clipboard (`^v`) to a picked visible window (same as D2C submit menu **[W]**). Shows `⏳ Scanning visible windows...` while building the picker, then after pick: **Y** = paste + Enter, **Esc** = abort, ~3s timeout = paste only; loading bar through activate/focus/paste/send, then a completion hold (`✅ Pasted` / `✅ Sent`) until safe to continue. Focuses a learned main text field when saved; otherwise prompts **Y**/**N** to persist the focused field. See [paste-field-mapping.md](paste-field-mapping.md). |
| `#!+U` then **L** twice | Same from hotstring selector.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |

## Files and entry points

- **Utils.ahk**: `~#!+0`, `DictationGeminiConfirm_*`, `GeminiDelayedSubmitFlow`, `GeminiFinalizeSubmit`, `Dictation_ShowVisiblePasteSelector`, `CursorTransfer_ActivateFocusPaste` (optional second arg restores focus to the anchored window after paste + Enter).
- **Gemini.ahk**: `GeminiDelayedSubmitMonitor` (response done detection), legacy “Copy response?” banner, Copy/Read/Favorite actions.
