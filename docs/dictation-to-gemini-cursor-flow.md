...
- [prompt-data-output-and-finance-packs.md](prompt-data-output-and-finance-packs.md) — Prompt Manager `ExpectsDataOutput` / `DataOutputFormat`, injected AIB delivery contract, finance/mnemonic `.txt` packs and import. Send dictation **[D]/[G]/[A]/[T]** load prompts by char and honor that metadata (file vs code is not controlled by `.txt` body prose alone).
