...
| `Utils.ahk` (`D2C_FlowManager.DoCopyCore`)                                   | **Response ready** banner: user presses **P** (Copy), **Y** (Desktop), **F** (Favorite), **R** (Read), **W** (Paste window), or **O** (Clip Angel) — each copies first | Ensure Gemini is active, copy the last Gemini response to the clipboard (with `#\!+P`-style quality gates); then run the chosen destination (Copy-only / Desktop / Favorite / Read / visible paste / Clip Angel) | **Outbound** (original → Gemini for copy path): play `pre-movement.wav`, wait 2s, then copy |
