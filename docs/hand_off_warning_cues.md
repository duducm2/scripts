...
| `Utils.ahk` (`D2C_FlowManager.DoCopyCore`)                                                                                      | **Response ready** banner: user presses **Y** (Desktop), **F** (Favorite), **C** (Transfer), **R** (Read), **W** (Paste window), or **O** (Clip Angel) — each copies first | Ensure Gemini is active, copy the last Gemini response to the clipboard; then run the chosen destination (Desktop / Favorite / Transfer / Read / visible paste / Clip Angel) | **Outbound** (original → Gemini for copy path): play `pre-movement.wav`, wait 2s, then copy            |
