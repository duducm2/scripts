...
| `#!+w` (`WM_WindowTools_ShowMenu`)                                                                                                                                   | **Interactive Input** menu via `StandardLoadingBar_ShowWithKeys` (`[1]` lone-window maximize per monitor, `[2]` minimized background list); option 2 opens a dark **numbered text selector** (`WM_ShowMinimizedBackgroundList`: keys `1`–`9`, `0` = 10th, then `a`–`z` to close windows; list refreshes after each close; Esc cancels); empty list uses **Information Only** `ShowCenteredOverlay_Utils` with `BANNER_ACCENT_INFO` |
