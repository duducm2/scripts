...
| `#!+w` (`WM_WindowTools_ShowMenu`)                                                                                                                                   | **Interactive Input** menu via `StandardLoadingBar_ShowWithKeys` (`[1]` lone-window maximize per monitor, `[2]` minimized background list); option 2 opens a dark selector-style **ListView** GUI (`WM_ShowMinimizedBackgroundList`); empty list uses **Information Only** `ShowCenteredOverlay_Utils` with `BANNER_ACCENT_INFO` |
