...
- **Translucency:** ~60% window alpha (153/255) via `WinSetTransparent(LANGUAGE_FLAG_ALPHA, flagGui)` applied after the final visible `Show("NA")`. Tunable through the `LANGUAGE_FLAG_ALPHA` global (255 = fully opaque, 0 = invisible).
...
| `LanguageFlag_Show(slot)` / `LanguageFlag_Hide()`           | **Persistent Indicator**: pinned, borderless flag (or EN/PT fallback) at ~60% window alpha (`LANGUAGE_FLAG_ALPHA`, 153/255) in the bottom-right of the active monitor (slot 3 = UK, slot 4 = Brazil); follows foreground window across monitors; independent of `StandardLoadingBar_*` |
...
| 3171–3285 | `LanguageFlag_GetImagePath`, `LanguageFlag_Show`, `LanguageFlag_Hide`, `LanguageFlag_RepositionToActiveMonitor`, `LanguageFlag_TrackTick`, `LanguageFlag_InitFromPersistedSlot` (Persistent Indicator; borderless, no margins, ~60% window alpha via `LANGUAGE_FLAG_ALPHA`, bottom-right anchor, independent track timer, EN/PT fallback if images missing) |
