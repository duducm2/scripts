...
- **Digit/letter keys (`1`, `2`, `Y`, …):** `ShowWithKeys` registers hook hotkeys as `$*key` (and `$*Numpad1`–`$*Numpad9` for digits) at `#InputLevel 10` and runs **`StandardLoadingBar_KeysSelectionPoll`** (edge-triggered `GetKeyState`, including `Numpad1`–`Numpad9`) so choices work when another script or the foreground app would otherwise consume the key. After the chord is released, digit keys are waited up, then each key’s `wasDown` is seeded from the current physical state so a held key cannot fire a spurious first edge. Poll ignores edges while Win/Ctrl/Alt are held (Shift is not checked here because `WaitForTriggerKeyRelease` already waited for it). **Digit detection:** use `StandardLoadingBar_IsDigitKey()` / `Ord()` — never `key >= "0"` on arbitrary slot letters (`a`–`z`, `N`, `Y`, etc.); in AHK v2 that throws and aborts `ShowWithKeys` before the timeout callback runs (e.g. blackout countdown).
...
| `#!+w` (`WM_WindowTools_ShowMenu`)                                                                                                                                   | **Interactive Input** menu via `StandardLoadingBar_ShowWithKeys` (`[1]` lone-window maximize per monitor, `[2]` minimized background list); option 2 opens a dark **numbered text selector** (`WM_ShowMinimizedBackgroundList`: keys `1`–`9`, `0` = 10th, then `a`–`z` to close windows; after each key the list **re-scans** (`WM_CollectBackgroundWindows`), drops the closed HWND, **re-renders** with keys renumbered from `[1]`, and keeps the modal open until empty or Esc; titles containing configured substrings are excluded (built-in defaults plus `data/wm_background_excludes.ini` `[Excludes]` `TitleContains`, case-insensitive partial match — e.g. IT Workplace, Drafts Monitor, Form1, Screenpresso); **`[A]`** opens an add-to-exclude picker (digits `1`–`9`, `0` = 10th) that appends the chosen window's **full title** to the INI and refreshes the list; **`[O]`** arms open mode (next digit restores/activates that window and **dismisses** the modal; digits otherwise close in-place and refresh); selection keys use **`#HotIf` + `$*` hook hotkeys** at `#InputLevel 10` plus an edge-triggered **keys poll** (same idea as `StandardLoadingBar_KeysSelectionPoll`, so slot `1` still works when a global `Hotkey("1")` exists elsewhere); digits/letters do not pass through to focused editors when unmodified; numpad digits included; keys pass through when Ctrl/Alt/Win/Shift are held so `^!#` monitor chords still work; modal uses **active-monitor tracking** (`WM_MinimizedList_TrackActiveMonitorTick`, 115 ms — same as `trackActiveMonitor` / `StudyTopicSelector`) so it stays centered on the monitor of the foreground window while `MonitorActiveWindow` recenters the cursor; Esc cancels via `g_OnEscapePressed`, `$*Escape` at `#InputLevel 10`, Esc poll, Gui `Escape`, and `.cursor/wm_minimized_list_open` IPC so **AppLaunchers** `Utils_GlobalEscapeHandler` requests close instead of forwarding Escape); empty list uses **Information Only** `ShowCenteredOverlay_Utils` with `BANNER_ACCENT_INFO` |
