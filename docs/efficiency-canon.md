...
- **Fill-on-close (AutoSlot):** shell `WINDOWDESTROYED` primary + WinEvent DESTROY secondary (deduped); cheap top-level filter before `MonitorFromWindow`; debounce + **per-monitor cooldown**; if occupancy 1 and companion already max/work-area-filled → no-op (skip `WM_CollectBackgroundWindows`); else promote first background candidate (maximize or gapless 50/50 **without** strict validate); no background → heal companion once; suppress re-place via `g_AutoSlotRecent`. New-window `AutoSlot_Place` still runs strict bipartition validate.
