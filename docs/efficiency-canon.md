...
- **AutoSlot free-capacity (current):** no background import on close/move/minimize — heal leftover companion only. Explicit **Ctrl+Alt+Win+6** imports background into free capacity only (slotted windows never relocate): empty → BG; lone half → 50/50 with BG when candidates exist (else no-op); lone maximized → 50/50 with BG. See [`docs/canon/windows-rearrange.md`](canon/windows-rearrange.md).
