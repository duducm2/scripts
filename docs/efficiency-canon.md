...
- **Prompt Manager `[Y]` auto-send (all companions):** Shared stages — attach idle → paste → send ready → Enter/submit → Stop confirm. Adapters for `gemini` / `enterprise` / `copilot` (UIA root, Send, composer text, generating). Full write-up: [prompt-paste-companion-readiness.md](prompt-paste-companion-readiness.md). Consumer Gemini scopes UIA to Document/`chat-app`; chips via ClassName `new-file-preview-file` / filename extensions (`PromptContext_CountFileChips`). **Attach idle** (`PROMPT_PASTE_ATTACH_IDLE_FAST_PATH`): chips + no ProgressBar stable (ProgressBar over sticky upload Text). **Send ready** (`PROMPT_PASTE_USE_FAST_READY_PROBE`): after attach, light poll; wait capped at 500ms then force-submit. **Submit:** `PROMPT_PASTE_SUBMIT_VIA_ENTER` (default true; alias `PROMPT_PASTE_GEMINI_SUBMIT_VIA_ENTER`) uses `{Enter}` for all peers; confirm capped at 1.5s. Optional **`PROMPT_PASTE_READY_TIMING`**. **Rollback:** set either Enter flag false → UIA `TrySubmit`; `PROMPT_PASTE_USE_FAST_READY_PROBE := false` → two-gate wait; `PROMPT_PASTE_ATTACH_IDLE_FAST_PATH := false` → unconditional `minMs` attach wait; both `PROMPT_PASTE_USE_STABLE_SEND_READY` / `PROMPT_PASTE_USE_CHIP_READY` false → single-poll legacy wait.
...
- **Shared automation open:** [`ClipAngel_OpenForAutomation`](../Utils/clip_angel_activate.ahk) (`mode` all|favorites, optional `suppressVisual`) = one show/layout + [`ClipAngel_ApplyMarkFilterMode`](../Utils/clip_angel_favorite.ahk) + [`ClipAngel_FastEnsureRow0`](../Utils/clip_angel_favorite.ahk) (`^{Home}` only). Used by Alt+P/B, favorite mark, paste/export/`#!+7`, and UI-merge fallback. **Single UIA `root`:** MarkFilter element is cached during filter polls; full [`ClipAngel_UiaEnsureRow0Selected`](../Utils/clip_angel_favorite.ahk) remains for restart/quality gates only.
- **AHK-owned Alt+P / Alt+B:** non-tilde `!p` / `!b` → `ClipAngel_OpenWithMarkFilter` → Loading Indication + `OpenForAutomation` → success overlay. **No** native Alt open; **no** Window-ribbon poll. Manual Ctrl+1–5 while focused still mean copy-then-minimize (`hotif_clipangel`). See [standard_information_display.md](standard_information_display.md).
- **Non-silent activate:** [`ActivateClipAngelWithFocusCorrection`](../Utils/clip_angel_activate.ahk) uses animated `StandardLoadingBar` (⏳); skips double activate when layout already activated.
- **Legacy native settle helpers:** [`CLIPANGEL_NATIVE_OPEN_SETTLE_MS`](../Utils/clip_angel_activate.ahk) / `EnsureForegroundAfterNativeOpen` remain unused by Alt+P/B.
...
- **Direct DB (pass 2):** [`Utils/bin/ClipAngelDb.exe`](../Utils/bin/ClipAngelDb.exe) (x86, [`Utils/src/ClipAngelDb.cs`](../Utils/src/ClipAngelDb.cs)) reads `%LOCALAPPDATA%\ClipAngel\db.sqlite` with ClipAngel’s hardcoded SQLite password `Magic67234784` (from upstream `Main.cs`). **Read-only only** — writes lock while ClipAngel runs (`journal_mode=OFF`). Wrapper: [`Utils/clip_angel_db.ahk`](../Utils/clip_angel_db.ahk). Commands: `maxid`, `waitnew`, `newest`, `isfav`, `mergepayload`. Any failure → exit 2 / empty string → UIA fallback. Rebuild: `Utils/src/build_clipangeldb.ps1`.
- **Ingest wait:** [`ClipAngel_WaitForClipboardIngest`](../Utils/clip_angel_favorite.ahk) prefers `waitnew` when a pre-copy `prevId` is passed; else polls `newest` title vs clipboard; UIA title poll remains fallback (cap 600 ms).
- **WM_HOTKEY posting:** [`ClipAngel_PostHotkey`](../Utils/clip_angel_activate.ahk) posts `0x0312` to all `ClipAngel.exe` windows (hidden hook window dispatches). Used for Ctrl+Alt+V/B paste-and-select (next/previous) instead of `SendInput` when possible; SendInput fallback if no window.
- **Constant Pasting (`Shift+P` / `Shift+B`):** delimiter ListView, then loop keeps the **paste target** foreground and fires native hotkeys via `PostHotkey` — **Shift+P ↓ → Ctrl+Alt+B** (paste+previous / down the list), **Shift+B ↑ → Ctrl+Alt+V** (paste+next / toward top). No per-clip `ActivateClipAngel` / Enter / UIA row walk. Light UIA peek only for end-of-list (selection unchanged after paste). Gap remains interruptible; stop with the same Shift+P/B.
- **Mark favorite (`^!#4` / `#!+.`):** DB `newest` pre-check skips open when already favorite; otherwise one `BeginFavoriteSuppress` via `OpenForAutomation(..., suppressVisual := true)`; Clip > Mark favorite UIA only (never bare `{Alt}` / `!q`); post-toggle `isfav` confirm. `#!+.` snapshots `maxid` before `^c`, then `MarkLastClipAsFavorite("first", true, prevId)`.
- **Merge:** [`MergeNonFavoriteClips`](../Utils/clip_angel_merge.ahk) prefers DB `mergepayload` → set clipboard → ingest wait (no window). UI path kept as `MergeNonFavoriteClips_ViaUI`.
- **Fast copy:** `FAST_COPY_DEBUG := false`; NDJSON only when enabled, log under `A_ScriptDir`. Post-paste uses `ClipWait` then Gemini upload idle (minNoIndicatorMs 2600 when no indicator). Sequential paste uses `ClipAngel_PostHotkey` for `^!b`.
- **Rollback:** restore settle 300 / remove fast path in `ClipAngel_ApplyLayoutOnMonitor` if maximize races; disable DB helper (rename/remove exe) to force UIA paths; restore `SendInput "^!b"` / Constant Paste Activate+Enter path if WM_HOTKEY posting races; restore `~!p`/`~!b` if AHK-owned Alt+P/B regresses.
...
- **Post-import SPA reload (required):** after successful pack/plan/quick-image writes, AHK calls `Palace_InvalidateServerCache` (`POST /api/reload`) **and** `Palace_RefreshRunningWebApp` (ControlSend `{F5}` to the open Memory Palace Chrome tab without activating it). Server cache alone is not enough — the SPA keeps `/api/state` in memory. `Palace_FinishImportSuccess` always triggers the client reload; quick-image uses `Palace_AfterDataWriteRefreshUi`. SPA also soft-refreshes on tab focus/visibility (debounced) and re-renders Quick Recall when the picker is open.
- **Post-import beast-thumb manifest:** after beast-writing pack imports (`nBeasts > 0`), AHK runs `Palace_RebuildBeastThumbManifest` (`rebuild_full_beast_thumb_manifest.py`) **before** SPA `{F5}`. Web icons are keyed by beast id in `beast-thumb-manifest.json`; skipping rebuild leaves new Custom beasts without adj/beast thumbs even when PNGs exist. Scripts open `beasts.csv` as `utf-8-sig` (BOM-safe).
