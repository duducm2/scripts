...
- **AutoSlot free-capacity (current):** no background import on close/move/minimize — heal leftover companion only. Explicit **Ctrl+Alt+Win+6** fills free capacity: empty → BG import; lone half + free slot → maximize that window; lone maximized → 50/50 with BG. See [`docs/canon/windows-rearrange.md`](canon/windows-rearrange.md).
