...
- **Single authority:** **`QuickLook_ApplyStudyLayout`** (activate, focus click, optional scroll) shared by **`QuickLook_OpenPath`** and the **`#!+X`** fast path when QuickLook is already running. After layout, **`AutoSlot_RequestPlaceCrossProcess(hwnd)`** asks WindowManagement AutoSlot to place into free capacity (empty maximize / free half 50/50) via same-process call or `.cursor\autoslot_place_request` IPC — QuickLook is often resident so shell CREATE/Place alone is unreliable.
