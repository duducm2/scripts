...
- **WM_HOTKEY posting:** [`ClipAngel_PostHotkey`](../Utils/clip_angel_activate.ahk) posts `0x0312` to all `ClipAngel.exe` windows (hidden hook window dispatches). Used for Ctrl+Alt+B incremental/decremental paste instead of `SendInput` when possible; SendInput fallback if no window.
...
- **Rollback:** restore settle 300 / remove fast path in `ClipAngel_ApplyLayoutOnMonitor` if maximize races; disable DB helper (rename/remove exe) to force UIA paths; restore `SendInput "^!b"` if WM_HOTKEY posting races; restore `~!p`/`~!b` if AHK-owned Alt+P/B regresses.
