...
- **Single authority:** **`QuickLook_ApplyStudyLayout`** (activate, focus click, optional scroll) shared by **`QuickLook_OpenPath`** and the **`#!+X`** fast path when QuickLook is already running. After layout, **`QuickLook_ScheduleAutoSlotPlace`** defers IPC place requests (~700 ms and ~1800 ms), re-resolving `ahk_exe QuickLook.exe` each time — QL often resets size after document paint, so an immediate place does not stick. WindowManagement AutoSlot polls `.cursor\autoslot_place_request` and **verify-retries** once (~800 ms) if the hwnd is neither work-area/max nor still in a snap pair.
...
- **AutoSlot free-capacity (current):** no background import on close/move/minimize — heal leftover companion only. Explicit **Ctrl+Alt+Win+Y** fills free capacity: empty → BG import; lone half + free slot → maximize that window; lone maximized → 50/50 with BG. See [`docs/canon/windows-rearrange.md`](canon/windows-rearrange.md).
