...
- **AHK-owned Alt+P / Alt+B:** [`ClipAngel_OpenWithMarkFilter`](../Utils/clip_angel_activate.ahk) via non-tilde `!p` / `!b` in [`WindowManagement/hotkeys.ahk`](../WindowManagement/hotkeys.ahk) (requires Clip Angel process; clear Clip Angel’s own global Alt+P/B). Flow: **`StandardLoadingBar_Show`** (⏳ Loading Indication; `centerOnHwnd` = prior app) → one show/layout (`ActivateClipAngelWithFocusCorrection` silent + `skipRow0`) → skip `ControlSend` when MarkFilter already matches → else **`ControlSend` Ctrl+1/2** + bounded poll (~300 ms) → one [`ClipAngel_FastEnsureRow0`](../Utils/clip_angel_favorite.ahk) reusing the same UIA `root` (full `UiaEnsureRow0Selected` only on fast-path miss) → Hide + short success overlay. **No** native Alt open; **no** Window-ribbon poll on this path. Manual Ctrl+1–5 while focused still mean copy-then-minimize (`hotif_clipangel`); filter apply uses ControlSend so HotIf does not steal it. See [standard_information_display.md](standard_information_display.md).
