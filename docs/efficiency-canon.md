...
- **Mark favorite hot path (2026):** [`MarkLastClipAsFavorite`](../Utils/clip_angel_favorite.ahk) uses **Clip > Mark favorite** UIA (`FindFirst` + bounded poll) — not `SendInput "!q"` (Alt steals Window ribbon). **Reuse one** `ElementFromHandle` root across Prepare / title-star / menu invoke. **Skip** Favorite-column `FindAll` (column absent in UIA tree). When not on Row 0: **UIA click** MainMenu `Window` then select Row 0 (condition wait for ribbon dismiss) before `^{Home}`. Title RTF star verifies when checkbox UIA is missing. No NDJSON / agent logs on this path.
- **Rollback:** restore settle 300 / remove fast path in `ClipAngel_ApplyLayoutOnMonitor` if maximize races on a specific DPI/monitor setup; restore fixed Sleep chains / per-poll layout in LeaveFavoritesFilter / WaitForListReady / HideWindow if CA UI races appear; restore Alt+Q-primary + Favorite-cell `FindAll` mark path only if menu Name localization misses.
