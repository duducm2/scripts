...
- **Shared automation open:** [`ClipAngel_OpenForAutomation`](../Utils/clip_angel_activate.ahk) (`mode` all|favorites, optional `suppressVisual`) = one show/layout + [`ClipAngel_ApplyMarkFilterMode`](../Utils/clip_angel_favorite.ahk) + one [`ClipAngel_FastEnsureRow0`](../Utils/clip_angel_favorite.ahk). Used by Alt+P/B, favorite mark, paste/export/`#!+7`, and merge. **Single UIA `root`:** [`ClipAngel_UiaEnsureRow0Selected`](../Utils/clip_angel_favorite.ahk) accepts optional `root`; MarkFilter element is cached during filter polls.
...
- **Condition waits over fixed Sleep:** ingest uses [`ClipAngel_WaitForClipboardIngest`](../Utils/clip_angel_favorite.ahk) (bounded poll, cap 600 ms). Menu opens use [`ClipAngel_UiaWaitMenuItem`](../Utils/clip_angel_favorite.ahk). Paste settle uses `ClipWait`. [`ClipAngel_HideWindow`](../Utils/clip_angel_favorite.ahk) polls iconic/hidden.
- **Mark favorite (`^!#4` / `#!+.`):** one `BeginFavoriteSuppress` via `OpenForAutomation(..., suppressVisual := true)`; Clip > Mark favorite UIA only (never bare `{Alt}` / `!q`). `#!+.` → `^c` + `MarkLastClipAsFavorite("first", true)` (no 1.2 s sleep chain).
- **Merge:** [`MergeNonFavoriteClips`](../Utils/clip_angel_merge.ahk) uses `OpenForAutomation` favorites then all-marks; no native `!b` / fixed 500–600 ms opens; Loading Indication throughout.
- **Fast copy:** `FAST_COPY_DEBUG := false`; NDJSON only when enabled, log under `A_ScriptDir`. Post-paste uses `ClipWait` then Gemini upload idle (minNoIndicatorMs 2600 when no indicator).
- **Rollback:** restore settle 300 / remove fast path in `ClipAngel_ApplyLayoutOnMonitor` if maximize races; restore fixed Sleep ingest / native Alt+P paste open if races appear; restore `~!p`/`~!b` if AHK-owned Alt+P/B regresses.
