...
- **Do** run **`Git: Stash`**, **`Git: Stash (Include Untracked)`** (fallback/recovery only), **`Git: Fetch`**, **`Git: Pull`**, and optional **`Git: Stash Pop`** via the command palette; **`Editor_FindQuickInputListItemExact` + Invoke/Click** — not `{Enter}` on fuzzy filter (VS Code lists both stash variants and may highlight the wrong row).
