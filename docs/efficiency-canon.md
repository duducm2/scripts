...
- **AHK-owned Alt+P / Alt+B:** [`ClipAngel_OpenWithMarkFilter`](../Utils/clip_angel_activate.ahk) via non-tilde `!p` / `!b` in [`WindowManagement/hotkeys.ahk`](../WindowManagement/hotkeys.ahk) (requires Clip Angel process; clear Clip Angel’s own global Alt+P/B). Flow: one show/layout (`ActivateClipAngelWithFocusCorrection` with `skipRow0`), **`ControlSend` Ctrl+1** (all marks) or **Ctrl+2** (favorites), bounded MarkFilter poll (~300 ms), one [`ClipAngel_UiaEnsureRow0Selected`](../Utils/clip_angel_favorite.ahk). **No** native Alt open; **no** Window-ribbon poll / `UiaFocusLooksLikeRibbonMenu` on this path. Manual Ctrl+1–5 while focused still mean copy-then-minimize (`hotif_clipangel`); filter apply uses ControlSend so HotIf does not steal it.
