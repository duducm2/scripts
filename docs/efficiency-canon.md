...
- **Prompt Manager `[Y]` auto-send:** [`PromptContext_WaitForSendReady`](../Utils/hotstring_selector_handlers_01.ahk) races **two independent gates** on the same bounded poll (AHK has no worker threads; first streak to `PROMPT_PASTE_SEND_READY_STABLE_POLLS` wins). **A (Send-stable):** scoped upload text gone + Send enabled + composer text; if `attachCount > 0` and no upload label, wait `PROMPT_PASTE_SEND_MIN_NO_INDICATOR_MS` before counting. **B (chips):** composer text + no ProgressBar in the search root +, when attaching, remove/close-file chip count ≥ `attachCount` (does not use Send.IsEnabled or upload labels). Banner phases: uploads → Send. **Rollback:** `PROMPT_PASTE_USE_STABLE_SEND_READY := false` and `PROMPT_PASTE_USE_CHIP_READY := false` restore single-poll legacy wait; either flag can be turned off alone.
