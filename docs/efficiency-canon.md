...
- **AHK-owned Alt+P / Alt+B:** [`ClipAngel_OpenWithMarkFilter`](../Utils/clip_angel_activate.ahk) via non-tilde `!p` / `!b` in [`WindowManagement/hotkeys.ahk`](../WindowManagement/hotkeys.ahk) (requires Clip Angel process; clear Clip Angel’s own global Alt+P/B). Flow: **`StandardLoadingBar_Show`** (⏳ Loading Indication; `centerOnHwnd` = prior app) → one show/layout (`ActivateClipAngelWithFocusCorrection` silent + `skipRow0`) → [`ClipAngel_ApplyMarkFilterMode`](../Utils/clip_angel_favorite.ahk) (grid focus + **SendInput** Ctrl+1/2, then ControlSend / Shift+P or MarkFilter UIA combo / List menu; **fail** if MarkFilter still wrong — do not claim success) → one [`ClipAngel_FastEnsureRow0`](../Utils/clip_angel_favorite.ahk) reusing the same UIA `root` → Hide + short success overlay. **No** native Alt open; **no** Window-ribbon poll on this path. Manual Ctrl+1–5 while focused still mean copy-then-minimize (`hotif_clipangel`); AHK-generated SendLevel 0 does not trip those hotkeys. See [standard_information_display.md](standard_information_display.md).
- **Non-silent activate:** [`ActivateClipAngelWithFocusCorrection`](../Utils/clip_angel_activate.ahk) uses animated `StandardLoadingBar` (⏳) when layout/show runs, not passive `ClipAngelBanner`.
- **Legacy native settle helpers:** [`CLIPANGEL_NATIVE_OPEN_SETTLE_MS`](../Utils/clip_angel_activate.ahk) / `EnsureForegroundAfterNativeOpen` remain in code but are unused by Alt+P/B after AHK ownership.
...
- **Mark favorite hot path (2026):** [`MarkLastClipAsFavorite`](../Utils/clip_angel_favorite.ahk) uses **Clip > Mark favorite** UIA (`FindFirst` + bounded poll) — not `SendInput "!q"` (Alt steals Window ribbon). **Reuse one** `ElementFromHandle` root across Prepare / title-star / menu invoke. **Skip** Favorite-column `FindAll` (column absent in UIA tree). Row 0 via SelectionItem / `^{Home}` ControlSend — **no** Window-ribbon recovery on open/Row0 helpers. Title RTF star verifies when checkbox UIA is missing. **Invisible suppress:** `CLIPANGEL_FAVORITE_SESSION_OPACITY := 0` + park off-screen (`BeginFavoriteSuppress` / `EndFavoriteSuppress`); no foreground activate; `ControlSend` via `ClipAngel_SendToHwnd` while suppress is active; minimize and wait hidden before clearing opacity. No NDJSON / agent logs on this path.
- **Rollback:** restore settle 300 / remove fast path in `ClipAngel_ApplyLayoutOnMonitor` if maximize races on a specific DPI/monitor setup; restore fixed Sleep chains / per-poll layout in LeaveFavoritesFilter / WaitForListReady / HideWindow if CA UI races appear; restore Alt+Q-primary + Favorite-cell `FindAll` mark path only if menu Name localization misses; restore on-screen transparent-only suppress (no off-screen park) if UIA fails off-screen on a specific build; restore `~!p`/`~!b` + native open if AHK-owned Alt+P/B regresses.
