...
- **Constant Pasting (`Shift+P` / `Shift+B`):** delimiter ListView, then loop keeps the **paste target** foreground and fires native **Ctrl+Alt+V** (paste+next) or **Ctrl+Alt+B** (paste+previous) via `PostHotkey` — no per-clip `ActivateClipAngel` / Enter / UIA row walk. Light UIA peek only for end-of-list (selection unchanged after paste). Gap remains interruptible; stop with the same Shift+P/B.
