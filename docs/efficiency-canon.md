...
- **Mark favorite hot path (2026):** [`MarkLastClipAsFavorite`](../Utils/clip_angel_favorite.ahk) uses **Clip > Mark favorite** UIA only — **never** bare `{Alt}`, **never** `!q` (both highlight Window ribbon). Keyboard Clip-menu open is a single `!c` chord. **Reuse one** `ElementFromHandle` root across Prepare / title-star / menu invoke. **Skip** Favorite-column `FindAll`. Row 0 via SelectionItem / `^{Home}` — **no** Window-menu helper. Title RTF star verifies when checkbox UIA is missing. **Invisible suppress:** opacity 0 + off-screen park; `ControlSend` via `ClipAngel_SendToHwnd` while suppress is active. No NDJSON on this path.
