...
- **Do** after a successful Alt+S whose repo is **this** scripts tree (`GetScriptsDirectory()`), call `QuickUpdateScripts()` so AHK relaunches from the pulled files. The `/Updated` overlay is the success signal. Do not send `Ctrl+Alt+Win+2` into the editor terminal. Other repos keep the Pull complete banner only.
