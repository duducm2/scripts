...
**Alt+S** in Cursor and VS Code is wrapped by AHK (`$!s` → `Editor_GitStashAndPull()` in [`Shift keys/cursor_predicates.ahk`](../Shift%20keys/cursor_predicates.ahk)) as **Git Stash, Fetch, and Pull** via **native CLI** ([`infra/tools/Editor-GitStashFetchPull.ps1`](../infra/tools/Editor-GitStashFetchPull.ps1)): resolves the active workspace git root from the editor process command line (and `status.scm.0` for multi-root), then runs one `git stash push -u` if dirty → `git fetch` → `git pull --ff-only` → optional `git stash pop`. Success is **not** git exit 0: the branch must be **0 commits behind upstream** and any stash must be restored. Shows a **Loading Indication** bar; per-command **120s timeout** plus an AHK outer wait; step-specific error banners on failure (including diverged branches, missing upstream, stash-pop conflicts). On full success (PS1 gates + AHK behind-count re-check): **✅ Pull complete** + **`pull-successful.wav`**. Does **not** use the command palette. Manual **Shift+P** remains separate. Listed on both overlays as `💾 [S][S]tash and Pull (Git) (ahk)`.
