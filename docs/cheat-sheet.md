...
| `Mobills`            | Chrome title contains `Mobills`. Daily entry is **not** a Shift key: dictation **[B]** asks Gemini to download `MOBILLS_V1.txt` on the Desktop; **Win+Alt+Shift+U** Macros **[m]** reads that file (or the newest matching `.txt`/`.ini`), confirms, enters transactions, then scrapes balances. |
