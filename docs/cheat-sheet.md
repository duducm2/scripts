...
**Alt+S** in Cursor and VS Code is wrapped by AHK (`$!s` → `Editor_GitStashAndPull()` in [`Shift keys/cursor_predicates.ahk`](../Shift%20keys/cursor_predicates.ahk)) as **Git Stash, Fetch, and Pull**: shows a **Loading Indication** bar (`StandardLoadingBar_*`) with milestone updates (stash → fetch → pull → sync), re-sends **Alt+S** (native **Git: Stash**), **UIA-waits** for the stash-message QuickInput, sleeps **400ms**, sends **Enter** for an empty stash message, runs **Git: Fetch** via the command palette, then sends **Shift+P** (native **Git: Pull**). Pull-pending state is read **after fetch** so the status bar reflects remote updates. When `status.scm.1` no longer shows pending **Pull N commits**, shows **✅ Pull complete** and plays **`pull-successful.wav`** (respects global sound toggle). Listed on both overlays as `💾 [S][S]tash and Pull (Git) (ahk)`.
