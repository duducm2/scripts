...
**Alt+S** in Cursor and VS Code is wrapped by AHK (`$!s` → `Editor_GitStashAndPull()` in [`Shift keys/cursor_predicates.ahk`](../Shift%20keys/cursor_predicates.ahk)) as **Git Stash, Fetch, and Pull** with **per-step quality gates**: shows a **Loading Indication** bar with milestone updates and elapsed seconds during waits; stashes via **`Git: Stash`** (UIA exact palette pick — not `{Enter}` fuzzy match, which conflates it with _Include Untracked_); falls back to **`Git: Stash (Include Untracked)`** when the working tree is still dirty; verifies stash (QuickInput closed + pending-changes drop or no working-tree blocker); then runs **Git: Fetch** and **Git: Pull** via the same UIA palette picker (not Shift+P — Shift+C in the editor is AHK-mapped to the palette). If pull hits a dirty working tree, the macro **re-stashes include-untracked and retries pull once**; on full success it runs **`Git: Stash Pop`** to restore local work (non-fatal if pop conflicts). **45s overall watchdog**; **one automatic retry** on fetch/pull failure (skipped after dirty-tree recovery); step-specific error banners on failure. On full success: **✅ Pull complete** + **`pull-successful.wav`**. Manual **Shift+P** pull (native keybinding) still requires a clean tree — use Alt+S first if you have untracked changes. Listed on both overlays as `💾 [S][S]tash and Pull (Git) (ahk)`.
