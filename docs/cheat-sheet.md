...
**Alt+S** in Cursor and VS Code is wrapped by AHK (`$!s` → `Editor_GitStashAndPull()` in [`Shift keys/cursor_predicates.ahk`](../Shift%20keys/cursor_predicates.ahk)) as **Git Stash and Pull**: shows a **Loading Indication** bar (`StandardLoadingBar_*`) with milestone updates (stash → pull → sync), re-sends **Alt+S** (native **Git: Stash**), **UIA-waits** for the stash-message QuickInput, sleeps **400ms**, sends **Enter** for an empty stash message, then sends **Shift+P** (native **Git: Pull**). When the status bar sync item (`status.scm.1`) no longer shows pending **Pull N commits**, shows **✅ Pull complete** and plays **`pull-successful.wav`** (respects global sound toggle). Listed on both overlays as `💾 [S][S]tash and Pull (Git) (ahk)`.
