...
**Alt+S** in Cursor and VS Code is wrapped by AHK (`$!s` → `Editor_GitStashAndPull()` in [`Shift keys/cursor_predicates.ahk`](../Shift%20keys/cursor_predicates.ahk)) as **Git Stash, Fetch, and Pull** via **native CLI** ([`infra/tools/Editor-GitStashFetchPull.ps1`](../infra/tools/Editor-GitStashFetchPull.ps1)): resolves the active workspace git root from the editor process command line (and `status.scm.0` for multi-root), then runs `git stash` → untracked fallback → `git fetch` → `git pull` → optional `git stash pop`. Shows a **Loading Indication** bar; **120s timeout**; step-specific error banners with git stderr on failure. On full success: **✅ Pull complete** + **`pull-successful.wav`**. Does **not** use the command palette. Manual **Shift+P** remains separate. Listed on both overlays as `💾 [S][S]tash and Pull (Git) (ahk)`.
