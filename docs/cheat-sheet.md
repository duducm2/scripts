...
| `Mobills`            | Chrome title contains `Mobills`. Daily entry is **not** a Shift key: dictation **[B]** builds a `MOBILLS_V1` file via the AI companion; **Win+Alt+Shift+U** Macros **[m]** reads the newest matching Desktop file, confirms, enters transactions, then scrapes balances. |
