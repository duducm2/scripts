...
**Alt+S** in Cursor and VS Code is wrapped by AHK (`$!s` → `Editor_GitStashAndPull()` in [`Shift keys/cursor_predicates.ahk`](../Shift%20keys/cursor_predicates.ahk)) as a **visible robot**: opens a **new editor terminal** (same as Shift+N) and types `git stash push -u` (only if there are stashable changes; dirty submodule content is skipped) → wait until index lock is gone and nothing stashable remains → `git fetch` → `git pull --ff-only` → `git stash pop` (only if that Alt+S stash was created). If stash reports `No local changes to save`, pull continues. Terminal prints `=== ROBOT … ===` for each step. On full success: **`pull-successful.wav`**. If the window is project-selector **[s] Scripts**, Alt+S then runs Quick Update (`QuickUpdateScripts()`), which relaunches AHK and shows **Scripts updated and relaunched**. Other repos show **✅ Pull complete** only. Does **not** use the command palette. Listed on both overlays as `💾 [S][S]tash and Pull (Git) (ahk)`.

**Shift+P** (`Editor_GitPullOrSync()`) is separate: opens Source Control, then tries UIA **Pull** → **Sync Changes** / **Synchronize Changes** → command palette **Git: Pull** → **Git: Sync**. Sync may also push. Listed as `⬇️ [P][P]ull or Sync (Git) (ahk)`.
