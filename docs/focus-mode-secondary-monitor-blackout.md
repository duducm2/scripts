...
- Activates target window, **`EnableFocusMode(keepIdx)`**, **`StartPdfFocusMonitor(targetHwnd, pdfFocusLossMode)`** — anchor is the study/PDF HWND.
...
- **Dwell:** same foreground HWND for **`FOCUS_BLACKOUT_DWELL_MS`** (20000 ms); crossing triggers countdown banner.
