...
(
    ...
    [Win+Alt+Shift+9] > 1× AI Quick Download (name Desktop file first, then configured click sequences → Desktop → rename → cut; manage via Win+Alt+Shift+U → Sequences) · 2× Audio / Bluetooth (1/B BT, 2/I Input, 3/O Output, 4/H Help, 5/G Ignored; letter jump; Enter default; Shift+D/E disable/enable; Shift+C/X connect/disconnect; Shift+I isolate; Shift+N ignore; Esc back) · hold 700ms+ Push scripts+notes (Utility [G])
    [Win+Alt+Shift+M] > Maximizes the current window
    [Win+Alt+Shift+Y] > 1× Focus Mode (black out other monitors) · 2× Main Repos (scripts + notes dirty files; [G]/[P] push like Utility [G] / #!+9 hold; [D] discard via reset --hard + clean -fd after MsgBox; [R] refresh; Esc close)
