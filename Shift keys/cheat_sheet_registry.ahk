...
(
    ...
    ⭐ [Ctrl+Enter] Copy selected clip (all types) → unmark favorite → paste
...
(
    ...
    🏠 [Esc] Exit project focus (Shift+Z), then clear selection
    ...
    🎯 [Z] Focus selected project — sections as bento (toggle)
...
(
    ...
    === Practice grid ===
    ☑ [Ctrl+Click] Toggle palace selection (does not open)
...
(
    ...
    [Win+Alt+Shift+P] > {AI_PROVIDER}: 1× copy last message; 2× (within 400ms) copy last code; then 5s [Y] Desktop [F] Favorite [R] Read (1× only) [W] Paste window [O] Clip Angel [N] No
    
    === DESKTOP ===
    [Win+Alt+Shift+O] > Newest Desktop item: 1× cut (return to previous window); 2× open; 3× paste copy to Desktop (clipboard file, Explorer selection, or Cursor/VS Code active editor file); hold 700ms+ copy path as text
    ...
    [Win+Alt+Shift+9] > 1× Push scripts+notes (Utility [G]) · 2× Audio / Bluetooth (1/B BT, 2/I Input, 3/O Output, 4/H Help, 5/G Ignored; letter jump; Enter default; Shift+D/E disable/enable; Shift+C/X connect/disconnect; Shift+I isolate; Shift+N ignore; Esc back) · hold 700ms+ AI Quick Download (name Desktop file first, then configured click sequences → Desktop → rename → cut; manage via Win+Alt+Shift+U → Sequences)
    [Win+Alt+Shift+M] > Maximizes the current window
    [Win+Alt+Shift+Y] > 1× Focus Mode (black out other monitors) · 2× Main Repos (scripts + notes dirty files; [G]/[P] push like Utility [G] / #!+9 tap; [D] discard via reset --hard + clean -fd after MsgBox; [R] refresh; Esc close)
    ...
    [Win+Alt+Shift+W] > Utility Shortcuts → Macros (same as #!+U then M); [1]/[2]/[3] set video/article/favorite from clipboard; [V]/[A]/[F] open stored video/article/favorite in new Chrome; [Q]/[M] AI companion Quick / Deep model; [B] Anti-Cat (lock notebook KB/trackpad only; no Bluetooth keyboards; unlock hold Esc 4s or [B] again)
